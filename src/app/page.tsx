    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <aside className="flex w-80 shrink-0 flex-col border-r border-border bg-bg-sidebar">
        ...
        <div className="flex items-center gap-2.5 border-b border-border px-4 py-3.5">
          ...
          <span className="text-sm font-semibold uppercase tracking-widest text-text-3">
            Generative UI
          </span>
        </div>

        <div className="border-b border-border">
          <p className="px-4 pt-3 pb-1.5 text-xs font-medium uppercase tracking-wider text-text-3">
          ...
          {QUICK_ACTIONS.map((a) => (
            <button
              ...
              disabled={generating}
              className="flex w-full items-center justify-between border-t border-border px-4 py-2.5
                         text-left hover:bg-surface disabled:opacity-30"
            >
              <span className="text-sm text-text-1">{a.label}</span>
              <span className="text-xs text-text-3">{a.description}</span>
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 space-y-3 overflow-y-auto p-3">
          {messages.length === 0 && (
            <div className="flex h-full flex-col items-center justify-center text-center">
              <p className="text-base text-text-2">{labels.home.title}</p>
              <p className="mt-1.5 text-sm leading-relaxed text-text-3">{labels.home.description}</p>
            </div>
          )}
              ...
              {msg.role === "user" ? (
                <div className="flex justify-end">
                  <p className="max-w-[85%] rounded-xl rounded-br-sm border border-accent/20
                                bg-accent/10 px-3 py-2 text-sm text-text-1">
                    {msg.content}
                  </p>
                </div>
              ) : (
                <div>
                  {msg.content && <p className="text-sm leading-relaxed text-text-2">{msg.content}</p>}
                  {msg.ui?.length ? (
                    <span className="mt-1 flex items-center gap-1.5 text-xs text-accent">
          ...
          {generating && (
            <p className="flex items-center gap-2 py-1 text-sm text-text-3">
        ...
        >
          <input
            ...
            disabled={generating}
            className="field-input flex-1 disabled:opacity-30"
          />
          <button
            type="submit"
      ...
      <main className="flex-1 overflow-y-auto p-8">
        {latestUI ? (
          ...
          <div className="flex h-full items-center justify-center">
            <p className="text-sm text-text-3">Your generated UI will appear here</p>
          </div>
        )}
      </main>
    </div>
